...
## 6. 実験的な 3D 描画対応
v0.4.2 移行に向けて、実験的な 3D 機能が実装されました。ただし、2D 描画で 3D をエミュレートする簡易的なものなので、次のような制約があります。
...
void Main()
{
	...
	Vec3 eyePositon(0, 4, 0);
	experimental::BasicCamera3D camera(Scene::Size(), fov, eyePositon, focusPosition);

	while (System::Update())
	{
		eyePositon = Cylindrical(8, Scene::Time() * 30_deg, Math::Sin(Scene::Time()) * 4);
...
void Main()
{
	constexpr double fov = 45_deg;
	constexpr Vec3 focusPosition(0, 0, 0);
	Vec3 eyePositon(0, 10, 0);
	experimental::BasicCamera3D camera(Scene::Size(), fov, eyePositon, focusPosition);

	while (System::Update())
	{
		eyePositon = Cylindrical(20, Scene::Time() * 30_deg, 8 + Periodic::Sine0_1(4s) * 8);
		camera.setView(eyePositon, focusPosition);
		const Mat4x4 mat = camera.getMat4x4();
