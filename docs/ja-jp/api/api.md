...
- 戻り値: 前のフレームで、ユーザーによりクリップボードが変更された場合`true`,それ以外の場合`false`

ユーザーによりクリップボードが変更されたか返します。

#### `bool Clipboard::GetText(String& text);`
- text: クリップボードのテキスト
- 戻り値: クリップボードに保存されている情報が、テキストの場合`true`,それ以外の場合`false`

クリップボードに保存されている情報が、テキストであるかを返します。

#### `bool Clipboard::GetImage(Image& image);`
- image: クリップボードの画像
- 戻り値: クリップボードに保存されている情報が、画像の場合`true`,それ以外の場合`false`

クリップボードに保存されている情報が、画像であるかを返します。

#### `bool Clipboard::GetFilePaths(Array<FilePath>& paths);`
- paths: クリップボードのファイルパス
- 戻り値: クリップボードに保存されている情報が、ファイルパスの場合`true`,それ以外の場合`false`

クリップボードに保存されている情報が、ファイルパスであるかを返します。ファイルパスはエクスプローラーなどで、ファイルを対象にコピーなどをした際にクリップボードに保存されます。
