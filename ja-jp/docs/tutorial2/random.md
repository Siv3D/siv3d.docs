...
- Siv3D は乱数に関する次のような機能を提供します。
    ...
    - 乱数を特定の分布に分散せる分布クラス
    - それらを組み合わせて実装される乱数関数
- 特に指定しない場合、Siv3D が内部でスレッドごとに確保するデフォルトの乱数エンジンが使われます
- それらのシード値は非決定的なハードウェアノイズによって決定されるため、プログラムを実行するたびに異なる乱数列を生成します
