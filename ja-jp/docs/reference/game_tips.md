...
色のみで区別する UI は、色の組み合わせによっては P 型や D 型の [色覚特性 :material-open-in-new:](https://cudo.jp/?page_id=540){:target="_blank"} を持つ人にとって操作が困難になります。色以外の要素（例えば形状やテキスト）でも区別できるようにするか、色の組み合わせを変えることで、色覚特性を持つ人にも操作しやすい UI にすることができます。
