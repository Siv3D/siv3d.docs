# 52. レンダーテクスチャ
図形・テクスチャ・フォントの描画先を、シーンの代わりにテクスチャにする方法を学びます。

## 52.1 レンダーテクスチャ

### 52.1.1 概要
- 図形・テクスチャ・フォントの描画は、デフォルトではシーンに対して行われます
- **レンダーテクスチャ** を使うと、プログラムで用意したテクスチャ（レンダーテクスチャ）を描画先にすることができます
...
- レンダーテクスチャは、**レンダーターゲットから解除されたあと**に、通常のテクスチャのように自身をシーンや別のレンダーターゲットに描画できます
- `RenderTexture` は `Texture` と同じような描画関数、操作関数を持っています
- `RenderTexture` の作成にはコストがかかるため、毎フレーム新しく作成するのではなく、事前に作成したものを使いまわしてください
- 図形・テクスチャ・フォントの `.draw()` による描画は、**チュートリアル ??** で学ぶ `Image` への書き込み (`.paint()` や `.overwrite()`) と異なり、GPU 上で実行されるため高速です
- レンダーステートも適用されるため、通常のシーンへの描画と同様に柔軟な描画ができます
...
## 52.2 クリアをしない使い方
- レンダーテクスチャに描く内容が変わらない場合、クリアを毎フレーム行う必要はありません
- 次のサンプルコードでは、最初のフレームで `RenderTexture` に描画を行い、以降は `RenderTexture` をクリアせずに毎フレーム描画することで、描画コストやクリアのコストを削減しています
...
- 初期状態が透過色のレンダーテクスチャに対して、デフォルトのブレンドステートで描画を行うと、レンダーテクスチャの内容の RGB 成分は更新される一方でアルファ成分は更新されません
- つまり全体が透過状態のままなので、そのレンダーテクスチャを描画しても何も表示されません
...
- これによって、レンダーテクスチャのアルファ成分が更新されるようになります
