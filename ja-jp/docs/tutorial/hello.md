# 1. はじめての Siv3D
Siv3D の最初のサンプルを改造して、Siv3D の機能を体験します。

## 1.1 最初のサンプルを改造する
Siv3D のインストール手順では、サンプルプログラムの実行を確認しました。
...
サンプルコード（下記）の中の :material-plus-circle: をクリックして説明を開き、手元でサンプルコードを改造しましょう。具体的な改造例を、サンプルコードの下に用意しています。
