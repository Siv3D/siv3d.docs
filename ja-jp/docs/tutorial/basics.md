...
- 通常の C++ プログラミングで必要な標準ライブラリのヘッダファイルは不要です
- Siv3D.hpp の中で `<algorithm>` や `<vector>` など、主要な標準ライブラリのヘッダファイルもインクルードされます
...
- 開発者は面倒な準備や後片付けを気にせず、やりたいことだけを `Main()` 関数の中に書けば OK ということです
- レストランの調理場で例えると：
...
- これで、グラフィックスの表示やユーザの入力の取得などを継続的に処理できるようになります
...
- Siv3D の公式サンプルは、すべて 3.5 の方法でプログラムを終了しています
	...
	- とくに理由がなければ `return;` の方法は使わないほうが何かと便利です
- 終了操作をカスタマイズする方法（++esc++ を押しても終了しないようにするなど）については、この先のチュートリアルで学びます
